...
describe('useAuditOrchestrator', () => {
  ...
  it('cancels pending and processing items without applying stale results', async () => {
    let releaseInvoice: (() => void) | undefined;
    detectDiningApplicationMock.mockRejectedValueOnce(new Error('skip application'));
    detectInvoiceMock.mockImplementationOnce(() => new Promise((resolve) => {
      releaseInvoice = () => resolve(makeExtracted());
    }));
    ...
    let auditItems: AuditItem[] = [
      { id: 'cancel-1', file: { name: 'cancel-1.png' } as File, status: 'pending' },
      { id: 'cancel-2', file: { name: 'cancel-2.png' } as File, status: 'pending' }
    ];
    const setAuditItems: React.Dispatch<React.SetStateAction<AuditItem[]>> = (updater) => {
      auditItems = typeof updater === 'function' ? updater(auditItems) : updater;
    };
    ...
    await act(async () => {
      processingPromise = result.current.processAuditItems([
        { id: 'cancel-1', file: { name: 'cancel-1.png' } as File },
        { id: 'cancel-2', file: { name: 'cancel-2.png' } as File }
      ]);
      await Promise.resolve();
    });

    expect(auditItems[0].status).toBe('processing');

    await act(async () => {
      result.current.cancelProcessing();
    });

    expect(auditItems[0].status).toBe('cancelled');
    expect(auditItems[1].status).toBe('cancelled');
    expect(result.current.isLoading).toBe(false);
    ...
    expect(auditItems[1].status).toBe('cancelled');
    expect(reportError).not.toHaveBeenCalled();
    expect(appendLog).toHaveBeenCalledWith({ level: 'info', message: '已中止当前审核任务' });
  });
});

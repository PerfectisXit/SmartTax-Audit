...
import { PROVIDER_DISPLAY_NAME, type OcrProvider } from './domain/provider';

const SettingsModal = React.lazy(() => import('./components/SettingsModal').then((m) => ({ default: m.SettingsModal })));
const UsageModal = React.lazy(() => import('./components/UsageModal').then((m) => ({ default: m.UsageModal })));
const TravelReimbursement = React.lazy(() => import('./components/TravelReimbursement').then((m) => ({ default: m.TravelReimbursement })));
...
const App: React.FC = () => {
  ...
  const getCurrentModelName = () => {
    return getModel(provider) || 'Default';
  };
          ...
          {currentTab === 'audit' ? (
            ...
            <div className="w-full">
               {(() => {
                    ...
                    return (
                       <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-300">
                          <p className="text-gray-500 mb-4">
                            {provider === 'chatgpt_web' ? '请先点击右上角设置并确认 ChatGPT 网页会话可用' : '请先点击右上角设置配置识别引擎'}
                          </p>
                  ...
                  return (
                     <Suspense fallback={<div className="text-center py-12 text-gray-500">正在加载差旅模块...</div>}>

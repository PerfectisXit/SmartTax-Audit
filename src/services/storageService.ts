...
export type OcrProviderKey = OcrProvider;
...
const LEGACY_OPENROUTER_API_KEY = 'openrouterApiKey';

const API_KEY_MAP: Partial<Record<OcrProviderKey, string>> = {
...
export const setApiKey = (p: OcrProviderKey, value: string) => {
  ...
  if (!key) {
    if (p === 'openrouter') {
      localStorage.removeItem(LEGACY_OPENROUTER_API_KEY);
    }
    return;
  }

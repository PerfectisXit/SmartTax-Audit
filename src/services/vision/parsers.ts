import { DiningApplicationData, ExtractedInvoiceData, InvoiceType, SmartFileAnalysis, TravelExpenseType, TravelMeta } from "../../types";
import { z } from 'zod';

const TravelMetaSchema = z.object({
...
const InvoiceRawSchema = z.object({
  ...
  travelMeta: TravelMetaSchema.optional()
}).passthrough();
...
const buildEmptyTravelMeta = (): TravelMeta => ({
  travelDate: '',
  travelTime: '',
  origin: '',
  destination: '',
...
const normalizeTravelMeta = (raw: unknown): TravelMeta => {
  const parsed = TravelMetaSchema.safeParse(raw);
  if (!parsed.success) return buildEmptyTravelMeta();
  return {
    travelDate: sanitizeText(parsed.data.travelDate),
    travelTime: sanitizeText(parsed.data.travelTime),
    origin: sanitizeText(parsed.data.origin),
    destination: sanitizeText(parsed.data.destination),
    tripNumber: sanitizeText(parsed.data.tripNumber),
    platform: sanitizeText(parsed.data.platform),
    orderRef: sanitizeText(parsed.data.orderRef),
    hotelName: sanitizeText(parsed.data.hotelName)
  };
};
...
export const normalizeInvoiceData = (raw: unknown): ExtractedInvoiceData => {
  const defaultResult: ExtractedInvoiceData = {
    ...
    items: [],
    travelMeta: buildEmptyTravelMeta()
  };
  ...
  return {
    ...
    travelMeta: normalizeTravelMeta(safeRaw.travelMeta)
  };
};
...
export const fallbackRegexParser = (text: string, defaultResult: ExtractedInvoiceData): ExtractedInvoiceData => {
  ...
  result.travelMeta = buildEmptyTravelMeta();
  return result;
};

...
\`\`\``;
    ...
    expect(parsed.travelMeta?.travelDate).toBe('2026-02-01');
    expect(parsed.travelMeta?.travelTime).toBe('08:35');
    expect(parsed.travelMeta?.tripNumber).toBe('MU1234');
  });
  ...
  it('falls back to regex parser when json parsing fails', () => {
    ...
    expect(parsed.travelMeta).toEqual({
      travelDate: '',
      travelTime: '',
      origin: '',
      destination: '',
  ...
  it('parses classifier output with tolerant schema normalization', () => {
    ...
    expect(parsed.extractedData?.travelMeta).toEqual({
      travelDate: '',
      travelTime: '',
      origin: '',
      destination: '',

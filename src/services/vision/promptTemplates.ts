...
4. **行程事件信息 (travelMeta)**: 仅在图片中明确可见时提取，不允许猜测；识别不到时返回空字符串。
   - travelDate: 实际出行日期，不是开票日期。
   - travelTime: 实际出发/乘车/下单时间，格式 HH:mm。
   - origin / destination: 出发地、到达地。
   - tripNumber: 航班号、车次号等。
   - platform: 平台名称，如滴滴/高德/美团/携程。
   - orderRef: 订单号、流水号、行程号等可用于区分同一事件的编号。
   - hotelName: 酒店名称。
...
2. **数据提取规则 (extractedData)**:
   - 如果是 **invoice** 类型，必须提取以下字段（与财务审计标准一致）：
     ...
     - travelMeta: 仅当图片中明确可见时提取；识别不到返回空字符串，不允许猜测。
       - travelDate: 实际出行日期
       - travelTime: 实际出发/下单时间，格式 HH:mm
       - origin / destination: 出发地与到达地
       - tripNumber: 航班号、车次号
       - platform: 平台名称
       - orderRef: 订单号、流水号、行程号
       - hotelName: 酒店名称

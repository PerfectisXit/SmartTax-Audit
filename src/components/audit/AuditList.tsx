        ...
        <div className="flex items-center gap-2">
          {isLoading && (
            <button
              ...
              className="text-xs text-rose-700 hover:text-rose-800 border border-rose-200 px-3 py-1.5 rounded-full bg-rose-50 apple-press"
            >
              中止本次审核
            </button>
          )}
        ...
        {items.map((item) => (
          <div key={item.id} className="flex items-center justify-between gap-3 p-2 rounded-xl border border-gray-100 bg-white/70 backdrop-blur apple-hover apple-press">
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded border ${
              ...
              item.status === 'cancelled' ? 'text-gray-600 bg-gray-100 border-gray-200' :

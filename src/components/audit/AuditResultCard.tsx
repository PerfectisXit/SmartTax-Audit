...
export const AuditResultCard: React.FC<AuditResultCardProps> = ({ item, diningApp }) => {
  const diningPlan = item.result?.diningPlan;
  const hasDiningApplication = Boolean(diningApp.data);
  const pairValidationEnabled = hasDiningApplication && item.diningIssues !== undefined;
  const pairIssues = item.diningIssues || [];
        ...
        <>
          {diningPlan && (
            <div className="p-4 rounded-xl border bg-blue-50 border-blue-100 text-blue-700">
              <div className="text-sm font-semibold">申请单填写建议（基于当前发票）</div>
              <div className="text-xs mt-2 space-y-1">
                <div>申请时间：{diningPlan.applicationDate}</div>
                <div>预计金额：¥{diningPlan.estimatedAmount.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}</div>
                <div>总人数：{diningPlan.totalPeople}</div>
                <div>招待人数：{diningPlan.guestCount}</div>
                <div>陪同人数：{diningPlan.staffCount}</div>
              </div>
            </div>
          )}

          <div className={`p-4 rounded-xl border ${
            !hasDiningApplication
              ? 'bg-gray-50 border-gray-100 text-gray-500'
              : !pairValidationEnabled
                ? 'bg-gray-50 border-gray-100 text-gray-600'
                : pairIssues.length > 0
                  ? 'bg-amber-50 border-amber-100 text-amber-700'
                  : 'bg-emerald-50 border-emerald-100 text-emerald-700'
          }`}>
            <div className="text-sm font-semibold">招待费申请单配对校验</div>
            {!hasDiningApplication && (
              <div className="text-xs mt-1">当前仅上传餐饮票，已按单票生成申请单填写建议。</div>
            )}
            {hasDiningApplication && !pairValidationEnabled && (
              <div className="text-xs mt-1">检测到非成对上传（需 1 张餐饮票 + 1 份申请单），暂不执行配对匹配校验。</div>
            )}
            {pairValidationEnabled && pairIssues.length > 0 && (
            <ul className="mt-1 text-xs list-disc pl-4 space-y-1">
                {pairIssues.map((msg, idx) => (
                <li key={idx}>{msg}</li>
              ))}
            </ul>
          )}

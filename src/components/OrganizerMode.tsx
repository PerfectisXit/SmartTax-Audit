...
export const OrganizerMode: React.FC<OrganizerModeProps> = ({ 
  ...
  const retrySmartFile = (id: string) => {
    setSmartFiles(prev => prev.map(f => (
      f.id === id
        ? { ...f, status: 'pending', error: undefined, analysis: undefined, processedFile: undefined }
        : f
    )));
  };
  ...
  const analyzeSingleFile = async (pending: SmartFile) => {
      try {
        ...
        setSmartFiles(prev => prev.map(f => f.id === pending.id ? { ...f, status: 'error', error: toFriendlyError(e.message) } : f));
    }
  };
                                                ...
                                                <div>
                                                    ...
                                                    {file.status === 'error' && (
                                                      <div className="mt-0.5 flex items-center gap-2">
                                                        <div className="text-[10px] text-red-500">{file.error}</div>
                                                        <button
                                                          onClick={() => retrySmartFile(file.id)}
                                                          className="text-[10px] text-blue-600 hover:text-blue-700 border border-blue-200 bg-blue-50 px-2 py-0.5 rounded-full apple-press"
                                                        >
                                                          重试
                                                        </button>
                                                      </div>

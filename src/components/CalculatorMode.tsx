...
import { TravelBatchItem, TravelDedupSummary, TravelExpenseType, TravelReport, SmartFile } from '../types';
...
import { analyzeTravelDuplicates } from '../services/travelDedupService';

const EMPTY_DEDUP_SUMMARY: TravelDedupSummary = {
  duplicateGroups: [],
  duplicateGroupCount: 0,
  estimatedSavings: 0
};

const normalizeSummary = (summary?: TravelDedupSummary | null): TravelDedupSummary => {
  if (!summary) return EMPTY_DEDUP_SUMMARY;
  return {
    duplicateGroups: Array.isArray(summary.duplicateGroups) ? summary.duplicateGroups : [],
    duplicateGroupCount: Number.isFinite(summary.duplicateGroupCount) ? Math.max(0, summary.duplicateGroupCount) : 0,
    estimatedSavings: Number.isFinite(summary.estimatedSavings) ? Math.max(0, summary.estimatedSavings) : 0
  };
};

const toFriendlyError = (message?: string) => {
  const raw = (message || '').trim();
  if (!raw) return '处理失败，请稍后重试';
  const normalized = raw.toLowerCase();
  if (normalized.includes('429') || normalized.includes('rate limit') || normalized.includes('tpm limit')) {
    return '接口触发限流，请稍后点击“重试”再次提交';
  }
  return raw;
};

const mergeDedupMeta = (base: TravelBatchItem[], incoming: TravelBatchItem[]): TravelBatchItem[] => {
  ...
  return base.map(item => {
    ...
    return {
      ...item,
      fileHash: next.fileHash ?? item.fileHash,
      duplicateGroupId: next.duplicateGroupId,
      duplicatePrimaryId: next.duplicatePrimaryId,
      duplicateReason: next.duplicateReason,
      duplicateConfidence: next.duplicateConfidence,
      duplicateStatus: next.duplicateStatus ?? item.duplicateStatus ?? 'none'
    };
  });
};

const hasDedupMetaChanged = (a: TravelBatchItem[], b: TravelBatchItem[]) => {
  ...
  for (const oldItem of a) {
    ...
    if (
      oldItem.fileHash !== newItem.fileHash ||
      oldItem.duplicateGroupId !== newItem.duplicateGroupId ||
      oldItem.duplicatePrimaryId !== newItem.duplicatePrimaryId ||
      oldItem.duplicateReason !== newItem.duplicateReason ||
      oldItem.duplicateConfidence !== newItem.duplicateConfidence ||
      oldItem.duplicateStatus !== newItem.duplicateStatus
    ) {
      return true;
    }
  }
...
export const CalculatorMode: React.FC<CalculatorModeProps> = ({ 
  ...
  const dedupTaskRef = useRef(0);
  const [dedupSummary, setDedupSummary] = useState<TravelDedupSummary>(EMPTY_DEDUP_SUMMARY);

  const recomputeDuplicates = async (items: TravelBatchItem[]) => {
    const taskId = ++dedupTaskRef.current;
    if (items.length === 0) {
      setDedupSummary(EMPTY_DEDUP_SUMMARY);
      return;
    }
    try {
      const result = await analyzeTravelDuplicates(items);
      if (taskId !== dedupTaskRef.current) return;
      const dedupedItems = mergeDedupMeta(items, Array.isArray(result?.items) ? result.items : []);
      const summary = normalizeSummary(result?.summary);
      setDedupSummary(summary);
      setCalcItems(prev => (hasDedupMetaChanged(prev, dedupedItems) ? mergeDedupMeta(prev, dedupedItems) : prev));
    } catch (error) {
      console.warn('Failed to recalculate duplicate items:', error);
      if (taskId !== dedupTaskRef.current) return;
      setDedupSummary(EMPTY_DEDUP_SUMMARY);
    }
  };

  const updateItemsAndRecompute = (updater: (prev: TravelBatchItem[]) => TravelBatchItem[]) => {
    setCalcItems(prev => {
      const next = updater(prev);
      queueMicrotask(() => {
        void recomputeDuplicates(next);
      });
      return next;
    });
  };
  ...
  useEffect(() => {
      if (initialSmartFiles && initialSmartFiles.length > 0) {
          ...
          void recomputeDuplicates(newItems);
      }
  ...
  useEffect(() => {
    const CONCURRENCY_LIMIT = provider === 'chatgpt_web' ? 1 : 3;
  ...
  const processSingleItem = async (pendingItem: TravelBatchItem) => {
      try {
        ...
        updateItemsAndRecompute(prev => prev.map(i => i.id === pendingItem.id ? {
            ...i,
            status: 'success',
            result: finalData,
         ...
         setCalcItems(prev => prev.map(i => i.id === pendingItem.id ? {
           ...i,
           status: 'error',
           error: toFriendlyError(e.message || 'Unknown error')
         } : i));
      }
  };
  ...
  const handleCalcFilesSelect = (files: File[]) => {
    ...
    updateItemsAndRecompute(prev => [...prev, ...newItems]);
  };

  const removeCalcItem = (id: string) => {
    updateItemsAndRecompute(prev => prev.filter(i => i.id !== id));
  };

  const retryCalcItem = (id: string) => {
    updateItemsAndRecompute(prev => prev.map(i => (
      i.id === id
        ? {
            ...i,
            status: 'pending',
  ...
  const updateDuplicateStatus = (id: string, status: 'confirmed_duplicate' | 'confirmed_distinct') => {
      updateItemsAndRecompute(prev => prev.map(i => i.id === id ? { ...i, duplicateStatus: status } : i));
  };
    ...
    <div className="grid lg:grid-cols-12 gap-8 items-start animate-fade-in pb-12">
           ...
           {calcItems.length > 0 && (
             <div className="mt-6 space-y-4">
               ...
               <div className="max-h-[700px] overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                 {pagedItems.map(item => {
                   const isDuplicatePrimary = Boolean(item.duplicateGroupId && item.duplicatePrimaryId === item.id);
                   const isDuplicateCandidate = item.duplicateStatus === 'pending_review' && Boolean(item.duplicateGroupId) && item.duplicatePrimaryId !== item.id;
                   const isDuplicateExcluded = item.duplicateStatus === 'confirmed_duplicate';
                   return (
                   <div key={item.id} className={`group relative bg-white border rounded-2xl p-4 transition-all duration-300 hover:shadow-md ${item.result?.isRefundDetected ? 'border-orange-200 bg-orange-50/30' : isDuplicateCandidate ? 'border-amber-200 bg-amber-50/20' : 'border-gray-100 hover:border-blue-200'}`}>
                          ...
                          <div className="flex-1 min-w-0">
                             <div className="flex justify-between items-start">
                                <div className="space-y-1">
                                    ...
                                    {item.result ? (
                                        <div className="flex items-center gap-2 text-xs text-gray-500">
                                            ...
                                            {isDuplicatePrimary && <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 px-1.5 py-0.5 rounded">已保留主凭证</span>}
                                            {isDuplicateCandidate && <span className="bg-amber-50 text-amber-700 border border-amber-200 px-1.5 py-0.5 rounded">疑似重复</span>}
                                            {item.duplicateStatus === 'confirmed_duplicate' && <span className="bg-gray-100 text-gray-600 border border-gray-200 px-1.5 py-0.5 rounded">重复已剔除</span>}
                                            {item.duplicateStatus === 'confirmed_distinct' && <span className="bg-blue-50 text-blue-700 border border-blue-200 px-1.5 py-0.5 rounded">已保留不同票据</span>}
                                        </div>
                                    ) : (
                                        <div className="text-xs">
                                            {item.status === 'processing' ? (
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button
                                                      onClick={() => retryCalcItem(item.id)}
                                                      className="px-2 py-0.5 rounded border border-red-200 bg-white text-red-600 hover:bg-red-50 transition apple-press"
                                                    >
                                                      重试
                                                    </button>
                                                </div>
                                ...
                                {item.result && (
                                    <div className="text-right">
                                        <div className={`text-base font-bold font-mono ${(item.refundStatus === 'confirmed_refund' || isDuplicateExcluded) ? 'line-through text-gray-400' : 'text-gray-900'}`}>
                                        ...
                                        {isDuplicateExcluded && <span className="text-[10px] text-gray-600 bg-gray-100 px-1 rounded ml-1">重复剔除</span>}
                                    </div>
                                )}
                             </div>
                             ...
                             {item.duplicateReason && (
                                 <div className="mt-2 text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2 py-1">
                                     重复依据：{item.duplicateReason}
                                 </div>
                             )}

                             {isDuplicateCandidate && (
                                 <div className="mt-3 p-3 bg-white border border-amber-200 rounded-xl shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-fade-in">
                                     <div className="flex items-center gap-2 text-xs text-amber-800">
                                       <span className="bg-amber-100 p-1 rounded text-amber-600">
                                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                                       </span>
                                       <span>系统检测到这张票与另一份文件属于同一出行事件，是否剔除副本？</span>
                                     </div>
                                     <div className="flex gap-2">
                                        <button
                                          onClick={() => updateDuplicateStatus(item.id, 'confirmed_duplicate')}
                                          className="px-3 py-1.5 text-xs bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition shadow-sm apple-press"
                                        >
                                           剔除副本
                                        </button>
                                        <button
                                          onClick={() => updateDuplicateStatus(item.id, 'confirmed_distinct')}
      ...
      <div className="lg:col-span-5 space-y-6">
         ...
         <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
            ...
            {dedupSummary.duplicateGroupCount > 0 && (
              <div className="mb-4 bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800">
                <div className="font-semibold">发现 {dedupSummary.duplicateGroupCount} 组疑似重复</div>
                <div className="mt-1">若按建议剔除，将减少 ¥{dedupSummary.estimatedSavings.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}</div>
              </div>
            )}

...
[![Version](https://img.shields.io/badge/version-v1.1.1-blue)](https://github.com/PerfectisXit/SmartTax-Audit/releases)
[![Release](https://img.shields.io/badge/release-v1.1.1-success)](https://github.com/PerfectisXit/SmartTax-Audit/releases/tag/v1.1.1)
![License](https://img.shields.io/badge/license-MIT-green)

SmartTax Audit Expert 是一款基于大语言模型（LLM）和视觉理解（VLM）技术的智能财务报销辅助工具。它能够自动识别、审核发票和差旅凭证，帮助财务人员和员工高效完成报销流程。项目支持多 provider，可按需切换云端 API 或本机 ChatGPT Web 自动化模式。
...
如需 OpenRouter 模型列表与代理能力，请另开终端启动后端：

```bash
npm run server
```

如需使用 ChatGPT Web provider（本机浏览器自动化），同样需要启动后端服务。

可选环境变量：
- `OCR_PYTHON_BIN`：OCR Python 可执行文件名，默认 `python`（如需可改成 `python3`）。
...
4.  **ChatGPT Web 模式说明**：该模式通过本机浏览器登录态自动化调用 ChatGPT 网页，不使用 OpenAI API Key，也不属于官方 API 接入方式。

## 📝 注意事项

*   **数据安全**：本项目纯前端运行（除可选的本地 Python OCR 服务外），图片直接发送至您配置的 LLM 服务商，不会存储在任何中间服务器。
*   **ChatGPT Web 特性**：这是非官方网页自动化入口，稳定性取决于 ChatGPT 网页结构与登录态状态变化。
*   **PDF 处理**：项目内置了 PDF.js 用于在浏览器端渲染 PDF 预览，无需后端转换。
*   **并发限制**：为避免触发 API 速率限制（Rate Limit），批量处理时采用了并发控制队列（默认并发数：3）。
*   **目录约定**：项目目录职责说明见 `docs/project-structure.md`。

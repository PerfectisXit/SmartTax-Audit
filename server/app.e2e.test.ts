...
describe('server e2e smoke', () => {
  beforeAll(async () => {
    ...
    app = mod.createApp({
      chatgptWebService: {
        getStatus: async () => ({ sessionReady: false, busy: false }),
        openSession: async () => ({ sessionReady: false, busy: false, opened: true }),
        runTask: async () => {
          const err: any = new Error('ChatGPT session required');
          err.status = 428;
  ...
  it('returns chatgpt-web status', async () => {
    const res = await request(app).get('/api/chatgpt-web/status');
    expect(res.status).toBe(200);
    expect(res.body.sessionReady).toBe(false);
    expect(res.body.busy).toBe(false);
  });

  it('validates chatgpt-web payload', async () => {
    const res = await request(app)
      .post('/api/chatgpt-web/run')
      .send({ prompt: 'missing taskType' });
    expect(res.status).toBe(400);
    expect(res.body.error).toBe('Invalid request payload');
  });

  it('returns session required for chatgpt-web run when not logged in', async () => {
    const res = await request(app)
      .post('/api/chatgpt-web/run')
      .send({
        taskType: 'invoice',
        prompt: 'ping',
        attachments: [{ name: 'upload.jpg', mimeType: 'image/jpeg', base64: 'aGVsbG8=' }]
      });
    expect(res.status).toBe(428);
    expect(res.body.error).toBe('ChatGPT session required');
  });
